    ...
    <div className="p-8 max-w-6xl mx-auto space-y-12">
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          <Badge variant="outline" className="text-mint border-mint/20">11 Experts Active</Badge>
...
function StatCard({ label, value, icon: Icon }: any) {
  return (
    <div className="p-5 bg-surface-1 border border-white/5 rounded-2xl space-y-2">
...
function PersonaCard({ manual }: { manual: any }) {
  return (
    <div className="group p-6 bg-surface-1 border border-white/5 rounded-2xl hover:border-mint/30 transition-all duration-300">
      <div className="flex items-start justify-between mb-4">
        <div className="space-y-1">
          <h3 className="text-sm font-bold text-white group-hover:text-mint transition-colors">{manual.name}</h3>
      ...
      <div className="mt-6 pt-6 border-t border-white/5 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <Button variant="ghost" size="sm" className="h-7 text-[9px] uppercase font-bold text-mint">View Logic</Button>
      </div>
    </div>
  );
}
